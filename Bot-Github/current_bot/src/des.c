...
const uint8_t ENCRYPTION_KEY[8] = {
    'L','*','#',')','@','!','&','8'
};
...
void KeySchedule(const uint8_t key[], uint8_t schedule[][6], uint32_t mode)
{
...
void DecryptData(uint8_t *data, uint32_t size, uint8_t *out, const uint8_t *key) {
	...
	for (uint32_t i = 0; i < (size / 8) * 8; i += 8) {
		Crypt(data + i, out + i, schedule);
	}
	
	// Copy any remaining bytes (unaltered)
    for (uint32_t i = (size / 8) * 8; i < size; i++) {
        out[i] = data[i];
    }
}
